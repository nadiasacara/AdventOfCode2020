CLASS ltcl_day14 DEFINITION FINAL FOR TESTING
  DURATION SHORT
  RISK LEVEL HARMLESS.
...
CLASS ltcl_day14 IMPLEMENTATION.
  ...
  METHOD first_1st_input.
    ...
    cut = NEW zcl_day14_ns( input ).
    cl_abap_unit_assert=>assert_equals( exp = 165 act = cut->first( ) ).
  ENDMETHOD.
  ...
  METHOD second_1st_input.
    ...
    cut = NEW zcl_day14_ns( input ).
    cl_abap_unit_assert=>assert_equals( exp = 208 act = cut->second( ) ).
  ENDMETHOD.
