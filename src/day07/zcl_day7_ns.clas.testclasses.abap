CLASS ltcl_day7 DEFINITION FINAL FOR TESTING
  DURATION SHORT
  RISK LEVEL HARMLESS.
  ...
  PRIVATE SECTION.
    ...
    METHODS first_short FOR TESTING.
    METHODS first_my_input FOR TESTING.

    METHODS second_32_short FOR TESTING.
    METHODS second_126_short FOR TESTING.
...
CLASS ltcl_day7 IMPLEMENTATION.
  ...
  METHOD first_short.
    ...
    cut = NEW zcl_day7_ns( input ).
    cl_abap_unit_assert=>assert_equals( exp = 4 act = cut->first( ) ).
  ENDMETHOD.

  METHOD first_my_input.
    cut = NEW zcl_day7_ns( get_my_input( ) ).
    cl_abap_unit_assert=>assert_equals( exp = 287 act = cut->first( ) ).
  ENDMETHOD.

  METHOD second_32_short.
    DATA(input) = VALUE string_table(
        ( |light red bags contain 1 bright white bag, 2 muted yellow bags.| )
        ( |dark orange bags contain 3 bright white bags, 4 muted yellow bags.| )
        ( |bright white bags contain 1 shiny gold bag.| )
    ...
    cut = NEW zcl_day7_ns( input ).
    cl_abap_unit_assert=>assert_equals( exp = 32 act = cut->second( ) ).
  ENDMETHOD.

  METHOD second_126_short.
    DATA(input) = VALUE string_table(
    ...
    cut = NEW zcl_day7_ns( input ).
    cl_abap_unit_assert=>assert_equals( exp = 126 act = cut->second( ) ).
  ENDMETHOD.

  METHOD second_my_input.
    cut = NEW zcl_day7_ns( get_my_input( ) ).
    cl_abap_unit_assert=>assert_equals( exp = 48160 act = cut->second( ) ).
  ENDMETHOD.
